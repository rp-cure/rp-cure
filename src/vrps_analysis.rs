use std::{vec, fs, ops::Index, path::Path, net::{IpAddr, Ipv4Addr, Ipv6Addr}, collections::{HashSet, HashMap}, str::FromStr};

use asn1::oid;
use bcder::{encode::{Values, PrimitiveContent}, Mode};
use bytes::Bytes;
use chrono::{Duration, Timelike};
use ipnet::{Ipv4Net, Ipv6Net, IpNet};
use rpki::{repository::{sigobj::SignedObject, roa::RouteOriginAttestation, resources::{IpBlock, Addr, self}, crypto::digest}, rrdp::Hash};
use serde_json::Value;
use crate::{asn1p::{self, Certificate}, cert, util, crl, roa};
use rpki_testing::{repository::{self, RepoConfig}, rp_interaction::RoaContents};
use rasn::{ber, der};
use sha256;
...
pub fn get_issuer_cert_uri(uri: &str) -> String{
    // let uri = "/home/nvogel/git/rpki-fuzzing/data/my.server.com/repo/newca/FB203CF59A2BAF2435CFD6E73A2F76786D797A28.mft";
...
pub fn load_objects(folder_uri: &str) -> (Option<String>, Option<String>, Vec<String>, Vec<String>){
    ...
    for u in fs::read_dir(folder_uri).unwrap().into_iter(){
        ...
        else if e.file_name().to_str().unwrap().ends_with("cer") || e.file_name().to_str().unwrap().ends_with("q7Tq8.roa") || e.file_name().to_str().unwrap().ends_with(".gbr"){
...
pub fn handle_cer_i(data: Vec<u8>, conf: &RepoConfig, parent_uri: Option<&str>, storage_uri: Option<&str>, ee_cert: bool) -> (Vec<u8>, Vec<(u8, IpBlock)>, i32){
    ...
    let key_int;
    if !ee_cert{
        (parsed, key_int) = cert::parse_and_sign(Bytes::from(data), &parent, conf, &child.get_pub_key(), "newca", parent_uri, storage_uri, ee_cert);
    }
    else{
        (parsed, key_int) = cert::parse_and_sign(Bytes::from(data), &child, conf, &child.get_pub_key(), "newca", parent_uri, storage_uri, ee_cert);
    }
...
pub fn handle_signed_object(data: &Bytes, storage_uri: &str, conf: &RepoConfig) -> Vec<u8>{
    ...
    let pk: &[u8] = &signer.get_pub_key().key_identifier().encode_ref().to_captured(Mode::Der).into_bytes()[2..];
    si.sid = Some(pk);
...
pub fn is_ber(content: Bytes) -> bool{
    ...
    let signed = SignedObject::decode(
        by, false,
    );
    if signed.is_ok(){
        let signed = SignedObject::decode(
...
pub fn handle_folder(folder_uri: &str) -> bool{
    ...
    let mft = mft.unwrap();
    let crl = crl.unwrap();
    let base = "/home/nvogel/Schreibtisch/testi/";
    let base = "/home/nvogel/Schreibtisch/vanilla_rps/routinator/dump/store/rrdp.ripe.net/rpki.ripe.net/";
...
pub fn load_vrps(self_run: bool) -> Vec<Vec<String>>{
    ...
    if !self_run{
        vrps_locations = vec!["/home/nvogel/Schreibtisch/vanilla_rps/routinator/vrps_routinator.txt".to_string(), "/home/nvogel/Schreibtisch/vanilla_rps/cfrpki/cmd/octorpki/vrps_octo.txt".to_string(), "/home/nvogel/Schreibtisch/vanilla_rps/FORT-validator/vrps_fort.txt".to_string(), "/home/nvogel/Schreibtisch/vanilla_rps/rpki-client-portable/output/csv".to_string()];
        // vrps_locations = vec!["/home/mirdita/vrps-fuzzing/routinator/routinator.json".to_string(), "/home/mirdita/vrps-fuzzing/octo/octo.json".to_string(), "/home/mirdita/vrps-fuzzing/fort/fort.csv".to_string(), "/home/mirdita/vrps-fuzzing/client/client.json".to_string()];
...
pub fn testi(){
    let uri = "/home/nvogel/Schreibtisch/vanilla_rps/rpki-client-portable/cache/repository.lacnic.net/rpki/lacnic/5a5c5891-0f6a-4c17-9e93-8c3a4bc7870c/e79c92da6cac6a9231e94f39988112836da43b04.roa";
    // let uri = "/home/nvogel/git/rpki-fuzzing/data/my.server.com/repo/newca/3138312e3231352e302e302f3234203d3e203232323232.roa";

    let content = fs::read(&uri).unwrap();
    let roa = asn1::parse_single::<asn1p::ContentInfo2>(&content);
    let by: &[u8] = &content.to_vec();

    // let by: &[u8] = &der_bytes.to_vec();


    let decoded: rasn::types::Any = ber::decode(&content).unwrap();
    let der_bytes = der::encode(&decoded).unwrap();
    let are_equal = content.len() == der_bytes.len() && content.iter().zip(der_bytes.iter()).all(|(a, b)| a == b);

    println!("Are the byte arrays different? {}", !are_equal);    if roa.is_err(){
        let signed = SignedObject::decode(
            by, false,
        ).unwrap();
        let b = signed.content().to_bytes();
        println!("{}", base64::encode(&b));
        let ro = asn1::parse_single::<asn1p::ROA>(&b).unwrap();
        ...
        println!("First attempt failed with error {}", roa.err().unwrap());
        // let r = rpki::repository::roa::Roa::decode(by, false).unwrap();

        // r.content();

        // let content = r.to_captured().into_bytes();
    
        let b = asn1::parse_single::<asn1p::ContentInfo2>(&by);
        if b.is_err(){
            println!("Second attempt failed with error {}", b.err().unwrap());
        }
        else{
            println!("Second attempt worked");
        }
    }
    else{
        println!("First attempt worked");
    }
}
...
pub fn load_roa_content(bases: Option<Vec<&str>>) -> HashMap<String, String>{
    let bases = match bases {
        Some(x) => x,
        None => {
            let base_routinator = "/home/nvogel/Schreibtisch/vanilla_rps/routinator/dump/store/";
            ...
            let ba = vec![base_octo, base_client, base_fort, base_routinator];
            ba
        }
...
pub fn analyse_cert(cert: asn1p::CertificateFull) -> CertCharacteristics{
    ...
    let iv = subject.clone().into_iter().filter(|x| (x.clone().into_iter().filter(|y| y.attrType.to_string() == "2.5.4.11" || y.attrType.to_string() == "2.5.4.10")).count() > 0 ).count() > 0;
...
pub fn find_object_differences_roa(){
    ...
    let base_client = "/home/nvogel/Schreibtisch/vanilla_rps/rpki-client-portable/cache/";
    let base_octo = "/home/nvogel/Schreibtisch/vanilla_rps/cfrpki/cmd/octorpki/cache/";
    let bases = vec![base_octo, base_client, base_fort, base_routinator];
...
pub fn test_it(){
    // let cont = fs::read("/home/nvogel/git/rpki-fuzzing/data/my.server.com/repo/newca/31302e302e302e302f3234203d3e203232323232.roa").unwrap();
    //cache/rsync.paas.rpki.ripe.net/repository/537459e7-2a83-43d1-9aa1-58417abac4b6/1/AS400229.roa
    let cont = fs::read("/home/nvogel/Schreibtisch/vanilla_rps/rpki-client-portable/cache/rsync.paas.rpki.ripe.net/repository/537459e7-2a83-43d1-9aa1-58417abac4b6/1/AS400229.roa").unwrap();

    // let new_obj = fix_econtent(Bytes::from(cont));
    // fs::write("/home/nvogel/git/rpki-fuzzing/newroas/adapted2.roa", new_obj).unwrap();


    // println!("{}", base64::encode(new_obj));
}