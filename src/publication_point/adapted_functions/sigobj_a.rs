...
use openssl::rsa::Rsa;
use openssl::sign::{Signer as OSigner, Verifier};
use rpki::repository::cert::{Cert, KeyUsage, Overclaim, ResourceCert, TbsCert};
use rpki::repository::crypto::PublicKey;
use rpki::repository::crypto::{
    Digest, DigestAlgorithm, KeyIdentifier, PublicKeyFormat, Signature, SignatureAlgorithm, Signer, SigningError,
};
use rpki::repository::oid;
use rpki::repository::resources::{AsBlocksBuilder, AsResources, AsResourcesBuilder, IpBlocksBuilder, IpResources, IpResourcesBuilder};
use rpki::repository::x509::{update_once, Name, Serial, Time, ValidationError, Validity};
use rpki::uri;
use std::str::FromStr;
use std::{cmp, io};
...
pub struct SignedObject {
    ...
    pub binary_signing_time: Option<u64>,
}
...
impl SignedAttrs {
    pub(crate) fn new(
        ...
        let mut signing_time = signing_time.map(|time| encode::sequence((oid::SIGNING_TIME.encode(), encode::set(time.encode_varied()))));
        let mut message_digest = Some(encode::sequence((oid::MESSAGE_DIGEST.encode(), encode::set(digest.encode_ref()))));
        let mut binary_signing_time =
            binary_signing_time.map(|time| encode::sequence((oid::AA_BINARY_SIGNING_TIME.encode(), encode::set(time.encode()))));
    ...
    fn take_message_digest<S: decode::Source>(
        ...
        update_once(message_digest, || cons.take_set(|cons| OctetString::take_from(cons)))
    }

    fn take_signing_time<S: decode::Source>(cons: &mut decode::Constructed<S>, signing_time: &mut Option<Time>) -> Result<(), S::Err> {
...
impl SignedObjectBuilder {
    pub fn new(serial_number: Serial, validity: Validity, crl_uri: uri::Rsync, ca_issuer: uri::Rsync, signed_object: uri::Rsync) -> Self {
        Self {
    ...
    pub fn finalize_alt<S: Signer>(
        ...
        //let key_id_new = ee_signer.create_key(PublicKeyFormat::Rsa).unwrap();
        // TODO CHANGE THIS BACK!!

        // let signature = ee_signer
        //     .sign(&ee_key, SignatureAlgorithm::default(), &signed_attrs.encode_verify())
        //     .unwrap();
        // let key_info = ee_signer.get_key_info(&ee_key).unwrap();

        let mut s = OSigner::new(OMessageDigest::sha256(), &ee_pkey).unwrap();
        s.update(&signed_attrs.encode_verify()).unwrap();
        let signature = s.sign_to_vec().unwrap();

        let signature = Signature::new(SignatureAlgorithm::default(), Bytes::from(signature));

        let sid = ee_key_id.key_identifier();
...
impl StartOfValue {
    fn new<V: encode::Values>(values: &V) -> Self {
        let mut res = StartOfValue { res: [0; 8], pos: 0 };
