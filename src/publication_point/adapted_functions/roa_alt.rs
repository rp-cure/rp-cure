...
use rpki::repository::crypto::{Signer, SigningError};
use rpki::repository::oid;
use rpki::repository::resources::{Addr, AddressFamily, Asn, IpResources, Prefix};
...
impl Roa {
    pub fn decode<S: decode::Source>(source: S, strict: bool) -> Result<Self, S::Err> {
        ...
        let content = signed.decode_content(|cons| RouteOriginAttestation::take_from(cons))?;
...
impl RouteOriginAttestation {
    ...
    pub fn iter(&self) -> impl Iterator<Item = FriendlyRoaIpAddress> + '_ {
        self.v4_addrs
            .iter()
            .map(|addr| FriendlyRoaIpAddress::new(addr, true))
            .chain(self.v6_addrs.iter().map(|addr| FriendlyRoaIpAddress::new(addr, false)))
    }
}

impl RouteOriginAttestation {
    fn take_from<S: decode::Source>(cons: &mut decode::Constructed<S>) -> Result<Self, S::Err> {
...
impl RoaIpAddresses {
    fn take_from<S: decode::Source>(cons: &mut decode::Constructed<S>, family: AddressFamily) -> Result<Self, S::Err> {
        cons.take_sequence(|cons| {
            cons.capture(|cons| {
    ...
    fn encode_ref_family(&self, family: [u8; 2]) -> Option<impl encode::Values + '_> {
        if self.0.is_empty() {
            None
        } else {
            Some(encode::sequence((OctetString::encode_slice(family), &self.0)))
        }
    }
}
...
impl<'a> Iterator for RoaIpAddressIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.0.is_empty() {
            ...
            Mode::Der
                .decode(&mut self.0, |cons| RoaIpAddress::take_opt_from_unchecked(cons))
                .unwrap()
        }
    }
}
...
impl RoaIpAddress {
    ...
    fn take_opt_from_unchecked<S: decode::Source>(cons: &mut decode::Constructed<S>) -> Result<Option<Self>, S::Err> {
    ...
    fn skip_opt_in<S: decode::Source>(cons: &mut decode::Constructed<S>, family: AddressFamily) -> Result<Option<()>, S::Err> {
        ...
        if let Some(max_length) = addr.max_length {
            if max_length > family.max_addr_len() || max_length < addr.prefix.addr_len() {
                return Err(decode::Malformed.into());
            }
        }
    ...
    fn encode(&self) -> impl encode::Values {
        encode::sequence((self.prefix.encode(), self.max_length.map(|v| v.encode())))
    }
}
...
impl FriendlyRoaIpAddress {
    ...
    pub fn max_length(self) -> u8 {
        self.addr.max_length.unwrap_or_else(|| self.addr.prefix.addr_len())
    }
}
...
impl RoaBuilder {
    pub fn new(as_id: Asn) -> Self {
        Self::with_addresses(as_id, RoaIpAddressesBuilder::new(), RoaIpAddressesBuilder::new())
    }

    pub fn with_addresses(as_id: Asn, v4: RoaIpAddressesBuilder, v6: RoaIpAddressesBuilder) -> Self {
        Self { as_id, v4, v6 }
    }
    ...
    pub fn push_v4_addr(&mut self, addr: Ipv4Addr, len: u8, max_len: Option<u8>) {
    ...
    pub fn push_v6_addr(&mut self, addr: Ipv6Addr, len: u8, max_len: Option<u8>) {
...
impl RoaIpAddressesBuilder {
    pub fn new() -> Self {
        RoaIpAddressesBuilder { addrs: Vec::new() }
    }
    ...
    pub fn to_addresses(&self) -> RoaIpAddresses {
        RoaIpAddresses(if self.addrs.is_empty() {
    ...
    pub fn to_resources(&self) -> IpResources {
        IpResources::blocks(self.addrs.iter().map(|addr| addr.prefix.into()).collect())
    }

    pub fn encode_ref(&self) -> impl encode::Values + '_ {
        encode::sequence(encode::slice(self.addrs.as_slice(), |v: &RoaIpAddress| v.encode()))
    }
}
...
impl Extend<RoaIpAddress> for RoaIpAddressesBuilder {
    ...
    where
        T: IntoIterator<Item = RoaIpAddress>,
    {
...
mod signer_test {
    ...
    fn make_roa() -> Roa {
        ...
        let roa = roa
            .finalize(
                SignedObjectBuilder::new(12u64.into(), Validity::from_secs(86400), uri.clone(), uri.clone(), uri),
                &signer,
                &key,
            )
            .unwrap();
        ...
        let cert = cert.validate_ta(TalInfo::from_name("foo".into()).into_arc(), true).unwrap();
    ...
    fn serde_roa() {
        ...
        assert_eq!(roa.to_captured().into_bytes(), deser_roa.to_captured().into_bytes())
    }
}
