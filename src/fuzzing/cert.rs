...
use asn1::{self, BitString, GeneralizedTime, OctetStringEncoded, SequenceOf};
...
fn handle_subjectInfoAcces(raw_bytes: Vec<u8>, ca_name: &str, conf: &RepoConfig, own_uri: Option<&str>, is_ee: bool) -> Vec<u8> {
    ...
    if is_ee {
        ...
        all_fields.push(asn1p::InfoAccessField {
            identifier: identifier,
            val: Some(asn1::IA5String::new(&own_uri.unwrap()).unwrap()),
        });
        ...
        all_fields.push(asn1p::InfoAccessField {
            identifier: identifier,
            val: Some(asn1::IA5String::new(&repo_uri).unwrap()),
        });
        ...
        all_fields.push(asn1p::InfoAccessField {
            identifier: identifier,
            val: Some(asn1::IA5String::new(&mft_uri).unwrap()),
        });
        ...
        all_fields.push(asn1p::InfoAccessField {
            identifier: identifier,
            val: Some(asn1::IA5String::new(&notification_uri).unwrap()),
        });
    }
...
pub fn handle_crl_distribution_points(crl_uri: &str) -> Vec<u8> {
    ...
    let g = asn1p::GeneralName {
        UniformResourceIdentifier: Some(ia),
    };

    let d = asn1p::DistributionPointName { fullname: Some(g) };

    let x = asn1p::DistributionPoint {
        distributionPoint: Some(d),
    };
    ...
    let r = asn1p::CrlDistributionPoints {
        identifier: i,
        crlDistributionPoints: octetstr,
    };
...
pub fn handle_basicConstraints(is_ee: bool) -> Vec<u8> {
    ...
    let ex = asn1p::ExtensionValue {
        identifier: asn1::ObjectIdentifier::from_string("2.5.29.19").unwrap(),
        critical: true,
        value: b,
    };
...
pub fn handle_certificatePolicies() -> Vec<u8> {
    ...
    let pi = asn1p::PolicyInformation {
        policyIdentifier: p,
        policy: None,
    };
...
pub fn parse_and_sign(
    ...
    let new_key;
    if is_ee {
        new_key = Some(repository::make_cert_key(
            &(conf.BASE_KEY_DIR_l.clone() + &inte.to_string() + ".der"),
            "rsa",
        ));
    } else {
        new_key = None;
...
pub fn handle_serialized_object_inner(
    data: Vec<(String, Vec<u8>, Vec<u8>, Vec<u8>, Vec<u8>, String)>,
    conf: &RepoConfig,
    start_index: u32,
) {
...
pub fn create_objects(folder: String, max_file_amount: u16, dont_move: bool, oneshot: bool, amount: u32) {
    ...
    loop {
        let data = generate_from_files_plain(
            &folder, &conf, amount, &priv_keys, &pub_keys, &cert_keys, "cer", &roas, &crls, &mfts,
        );
